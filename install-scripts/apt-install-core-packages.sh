...
sudo apt install -y xorg x11-xserver-utils arandr autorandr dialog bspwm sxhkd sddm picom polybar xautolock xsecurelock network-manager git kitty psmisc rofi pcmanfm font-manager gvfs-backends nitrogen dunst cups system-config-printer xsensors smbclient cifs-utils lxpolkit xbacklight lxappearance htop mc xarchiver pulseaudio pulsemixer pulseaudio-utils bluez bluez-firmware bluez-tools bluez-alsa-utils xfce4-pulseaudio-plugin moc mpv npm cava curl tmux imagemagick sxiv scrot flameshot firefox-esr numlockx ranger neovim notify-osd neofetch flatpak timeshift rename gparted linux-cpupower xfce4-power-manager xfce4-settings bpytop
